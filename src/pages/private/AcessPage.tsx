import { Button, ShowIf } from 'components';
...
export function AcessPage({}: Props) {
  const dispatch = useDispatch();

  const { users, auth } = useAppSelector((state) => state.authReducer);

  React.useEffect(() => {
    if (users === null) dispatch(actionsAuth.fetch({}));
    return () => {};
  }, []);
  ...
  return (
    <main className="w-full h-full overflow-y-auto ">
      <header style={{ borderColor: '#DDDFE2' }} className="desk:border-b">
        <section style={{ maxWidth: '938px' }} className="w-full h-full mx-auto">
          <div className="w-full pt-10 pb-7 px-5 desk:px-0 flex flex-col desk:flex-row items-center justify-between gap-5">
            ...
            <Link to="/register-access" type="button" className="w-full desk:w-auto">
              <Button type="button" className="w-max px-6 py-2 bg-primary rounded-md mb-1 hidden desk:flex">
                <span className="font-semibold text-white text-base normal-case">Cadastrar novo acesso</span>
              </Button>
              ...
              >
                <div className="flex items-center gap-5">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <span className="font-medium text-black/80 text-[17px] normal-case">Cadastrar um novo acesso</span>
                  </div>
                </div>
          ...
          <div className="flex gap-7 px-5 desk:px-0">
            ...
            <button type="button" className="px-1 pb-2">
              <span className="font-semibold text-black/70 text-base">Todos acessos</span>
            </button>
          </div>
        </section>
      </header>
      ...
      <section style={{ maxWidth: '938px' }} className="w-full h-full pt-4 desk:pt-5 mx-auto flex flex-col">
        <ul className="px-5 desk:px-0 grid grid-cols-1 desk:grid-cols-5 gap-4 desk:gap-6">
          {users?.map((user) => {
            return (
              <li key={user._id} className="box bg-white rounded-lg">
                <div
                  className={`w-1.5 h-full rounded-l-lg absolute left-0 top-0 ${user.active ? 'bg-teal-500' : 'bg-primary'}`}
                />

                <ShowIf show={auth.type === 'admin'}>
                  <Link
                    to={`/register-access?id=${user._id}`}
                    state={{ user }}
                    className="p-1.5 rounded-md bg-slate-100 absolute top-2 right-2 z-50"
                  >
                ...
                <div className="pt-8 pb-4 desk:pt-11 desk:pb-8 flex justify-center">
                  <img src="/default-user.jpg" alt="user photo" className="w-16 h-16 rounded-lg" />
                </div>

                <div className="pl-7 pb-5 desk:pb-7 flex flex-col">
                  <span className="text-sm">{user.type === 'user' ? 'Usuário' : 'Admin'}</span>
                  <span className="text-base font-semibold text-black/80 capitalize">{user.name}</span>
                </div>
              </li>
            );
          })}
